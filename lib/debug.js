...
var Debug = Class.$extend({
    ...
    __init__: function(options={}) {
        ...
        this.$data.extraTagProvider = options.extraTagProvider;
    ...
    /**
     * A function returning extra tag to pass to sentry.
     *
     * @property extraTagProvider
     * @type Function
     */
    getExtraTagProvider: cu.getter,
    setExtraTagProvider: cu.setter,
    ...
    _configureSentry: function() {
        Sentry.init({ dsn: this.$data.sentryKey, release: this.$data.release });
    ...
    _logSentry: function(type, error, tags, extra) {
        ...
        if (this.$data.extraTagProvider) {
            tags = lodash.merge({}, this.$data.extraTagProvider(), tags);
        }
    ...
    _processEvent(event) {
        if (event && this.$data.extraDataProvider) {
            event.extra = lodash.merge({}, this.$data.extraDataProvider(), event.extra);
        }
        if (event && this.$data.extraTagProvider) {
            event.tags = lodash.merge({}, this.$data.extraTagProvider(), event.tags);
        }
        return event;
    },
