...
var SentryRRWeb = require("@sentry/rrweb");
...
var Debug = Class.$extend({
    ...
    __init__: function(options={}) {
        ...
        this.$data.useSentryRRWeb = options.enableSentryRRWeb;
     ...
     * @type Function
     */
    getExtraTags: cu.getter,
    setExtraTags: cu.setter,
    ...
    _configureSentry: function() {
        let integrations = [];
        if (this.$data.useSentryRRWeb) {
            integrations.push(new SentryRRWeb());
        }

        Sentry.init({ 
            dsn: this.$data.sentryKey, 
    ...
    _logSentry: function(type, error, tags, extra) {
        ...
        if (this.$data.extraTags instanceof Object) {
            tags = lodash.merge({}, this.$data.extraTags, tags);
        }
    ...
    _processEvent(event) {
        ...
        if (event && this.$data.extraTags) {
            event.tags = lodash.merge({}, this.$data.extraTags, event.tags);
        }
        return event;
    },
